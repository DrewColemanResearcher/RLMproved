...
class RLM:
    ...
    def _spawn_completion_context(self, prompt: str | dict[str, Any]):
        ...
        elif self.backend_kwargs:
            # Sub-LLM environment calls should always use legacy string completions.
            sub_backend_kwargs = self.backend_kwargs.copy()
            sub_backend_kwargs["method"] = "legacy"
            other_backend_client = get_client(self.backend, sub_backend_kwargs)
        ...
        with self._spawn_completion_context(prompt) as (lm_handler, environment):
            ...
            for i in range(self.max_iterations):
                ...
                current_prompt = message_history + [
                    build_user_prompt(root_prompt, i, context_count, history_count, self.important_information)
                ]
