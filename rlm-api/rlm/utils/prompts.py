...
RLM_RESPONSE_SCHEMA_NAME = "rlm_formatted_response"
RLM_RESPONSE_SCHEMA: dict[str, Any] = {
    "type": "object",
    ...
    "properties": {
        ...
        "done": {"type": "boolean"},
        "variable_with_result": {"type": "string"},
    },
    "required": ["explanation", "code_blocks", "done", "variable_with_result"],
}
...
class RLMStructuredResponse(BaseModel):
    ...
    variable_with_result: str


def build_rlm_response_format() -> dict[str, Any]:
    return {
        "type": "json_schema",
        "json_schema": {
            "name": RLM_RESPONSE_SCHEMA_NAME,
            "schema": RLM_RESPONSE_SCHEMA,
            "strict": True,
        },
    }


def build_rlm_responses_text_format() -> dict[str, Any]:
    return {"format": build_rlm_response_format()}
...
When you want to execute Python code in the REPL environment, place the code in a `code_blocks` entry as raw Python (no markdown fences). The examples below show the kinds of snippets you can include in that array. For example, say we want our recursive model to search for the magic number in the context (assuming the context is a string), and the context is very long, so we want to chunk it:
```repl
chunk = context[:10000]
...
def build_user_prompt(
    ...
    prompt = prompt + important_information
