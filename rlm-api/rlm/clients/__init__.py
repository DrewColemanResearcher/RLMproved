...
def get_client(
    ...
    if backend == "openai":
        ...
        method = backend_kwargs.pop("method", backend_kwargs.pop("aruba_api_mode", "parsed_completions"))
        if method == "parsed_responses":
            return ArubaClientResponses(**backend_kwargs)
        return ArubaClientChatCompletion(**backend_kwargs)
        ...
        backend_kwargs.setdefault("method", "parsed_completions")
        return OpenAIClient(**backend_kwargs)
    elif backend == "vercel":
        from rlm.clients.openai import OpenAIClient
